...
import styleGradient, {
  PluginOptions as StyleGradientOptions,
  parseGradient,
  toGradient,
  getValidDir,
  GRAD_DIRS,
...
const plugin = (editor: grapesjs.Editor, opts: PluginOptions = {}) => {
          ...
          if (
            (newTypeValue === BackgroundType.Image &&
              [
                PROPERTY_BG_COLOR,
                PROPERTY_BG_GRAD,
                PROPERTY_BG_GRAD_DIR,
                PROPERTY_BG_GRAD_TYPE,
              ].indexOf(propName) < 0) ||
            (newTypeValue === BackgroundType.Color &&
              propName === PROPERTY_BG_COLOR) ||
  ...
  editor.Styles.addBuiltIn("background", {
    ...
    layerLabel: (l: any, { values, property }: any) => {
      const opt = property
        .getProperty(PROPERTY_BG_TYPE)
        .getOption(values[PROPERTY_BG_TYPE]);
    ...
    fromStyle(style: Record<string, string>, { property, name }: any) {
      ...
      if (style[PROPERTY_IMAGE]) {
        ...
        layers = property
          .__splitStyleName(style, PROPERTY_IMAGE, sep)
        ...
        props.forEach((prop: any) => {
          ...
          property
            .__splitStyleName(style, propName, sep)
    ...
    toStyle(newValues: Record<string, string>) {
      ...
      if (type === BackgroundType.Color) {
        ...
        image =
          bgColor === DEFAULT_IMAGE
            ? DEFAULT_IMAGE
            : `linear-gradient(${bgColor} 0%, ${bgColor} 100%)`;
