            ...
            draggable(hints.find('.right-handle').get(0), {
                ...
                drag(uleft) {
                    try {
                        ...
                        let width = widthIframe + total * config.dragDampen;
                        let left = editor.Canvas.getOffset().left;
                        let res = true;

                        if (width > maxDeviceSize || width < config.minScreenSize) {
                            uleft = maxLeftPos;
                            res = false;
                        } else {
...
const draggable = (element: HTMLElement, opts: DraggableOptions) => {
    let pos1 = 0,
        pos2 = 0,
        pos3 = 0,
        pos4 = 0,
