...
export default (editor: grapesjs.Editor, config: PluginOptions = {}) => {
  ...
  log("Gradient: registering pickers 0/3")
  ...
  if (styleTypeId) {
    log("Gradient: registering styles 1/3")

    Styles.addType(styleTypeId, {
      create({ change }: any) {
        log("Gradient: creating gradient panel")
      ...
      emit({ updateStyle }: any, { partial, value }: any) {
        log("Gradient: emit gradient panel")
        updateStyle(value, { partial });
      },
      update({ value }: any) {
        log("Gradient: update gradient panel")
        const { gp } = this;
      ...
      destroy() {
        log("Gradient: destroy gradient panel")
  ...
  if (builtInType) {
    log("Gradient: registering builtin types 2/3")

    Styles.addBuiltIn(builtInType, {
      type: 'composite',
